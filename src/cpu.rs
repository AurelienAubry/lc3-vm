...
impl CPU {
    ...
    fn i_ld(&mut self, instruction: u16, bus: &Bus) -> Result<()> {
        ...
        let addr = self.reg.read_register(Register::PC) as u32 + pc_offset as u32;
        self.reg
            .write_register(dst_reg, bus.read_mem_word(addr as u16));

        self.reg.update_flags(dst_reg);
    ...
    fn i_st(&mut self, instruction: u16, bus: &mut Bus) -> Result<()> {
        ...
        bus.write_mem_word(
            self.reg.read_register(Register::PC) + pc_offset_9,
            self.reg.read_register(src_reg),
        );
        Ok(())
    }
    ...
    fn i_jsr(&mut self, instruction: u16) -> Result<()> {
        self.reg
            .write_register(Register::R7, self.reg.read_register(Register::PC));
        ...
        if register_bit == 0 {
            ...
            self.reg
                .write_register(Register::PC, self.reg.read_register(base_register));
        } else {
            // JSR
            let pc_offset = sign_extend(instruction & 0x7FF, 11);
            self.reg.increment_register(Register::PC, pc_offset);
        }
    ...
    fn i_and(&mut self, instruction: u16) -> Result<()> {
        ...
        if imm_flag == 1 {
            ...
            self.reg
                .write_register(dst_reg, imm & self.reg.read_register(sr1_reg));
        } else {
            let sr2_reg = register_from_u16(instruction & 0x7)?;
            self.reg.write_register(
                dst_reg,
                self.reg.read_register(sr1_reg) & self.reg.read_register(sr2_reg),
            );
        }
    ...
    fn i_ldr(&mut self, instruction: u16, bus: &Bus) -> Result<()> {
        ...
        let address = self.reg.read_register(src_reg) as u32 + offset_6 as u32;
        self.reg
            .write_register(dst_reg, bus.read_mem_word(address as u16));
    ...
    fn i_str(&mut self, instruction: u16, bus: &mut Bus) -> Result<()> {
        ...
        let addr = self.reg.read_register(base_reg) as u32 + offset_6 as u32;
        bus.write_mem_word(addr as u16, self.reg.read_register(src_reg));
    ...
    fn i_not(&mut self, instruction: u16) -> Result<()> {
        ...
        self.reg
            .write_register(dst_reg, !self.reg.read_register(src_reg));
    ...
    fn i_ldi(&mut self, instruction: u16, bus: &Bus) -> Result<()> {
        ...
        let mem_addr = self.reg.read_register(Register::PC) + pc_offset;
        let real_addr = bus.read_mem_word(bus.read_mem_word(mem_addr));
        self.reg.write_register(dst_reg, real_addr);
    ...
    fn i_sti(&mut self, instruction: u16, bus: &mut Bus) -> Result<()> {
        ...
        bus.write_mem_word(
            bus.read_mem_word(self.reg.read_register(Register::PC) + pc_offset_9),
            self.reg.read_register(src_reg),
        );
    ...
    fn i_jmp(&mut self, instruction: u16) -> Result<()> {
        ...
        self.reg
            .write_register(Register::PC, self.reg.read_register(base_reg));
    ...
    fn i_lea(&mut self, instruction: u16) -> Result<()> {
        ...
        self.reg
            .write_register(dst_reg, self.reg.read_register(Register::PC) + pc_offset_9);
        self.reg.update_flags(dst_reg);
